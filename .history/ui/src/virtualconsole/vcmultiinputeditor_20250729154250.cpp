    ...
    , m_widgets(widgets)
{
    ...
    // Manually connect the buttons to the slots
    connect(m_ui->m_buttonBox->button(QDialogButtonBox::Ok), &QPushButton::clicked, this, &VCMultiInputEditor::accept);
    connect(m_ui->m_buttonBox->button(QDialogButtonBox::Cancel), &QPushButton::clicked, this, &VCMultiInputEditor::reject);
...
void VCMultiInputEditor::fillTree()
{
    ...
    commonInputs = firstWidget->findChildren<QLCInputSource*>();

    for (int i = 1; i < m_widgets.count(); ++i)
    {
        QHash<quint8, QSharedPointer<QLCInputSource>> widgetInputs = m_widgets.at(i)->findChildren<QLCInputSource*>();
