...
public:
    ...
    static QStringList inputSourceNames(int type);
>>>>>>>
protected:
    int m_type;
...
public:
    ...
    /**
     * Get the number of available input sources
     */
    int inputSourcesCount() const;

    /**
     * Get a list of all the assigned input sources
     */
    QList<QSharedPointer<QLCInputSource>> inputSources() const;
>>>>>>>

    /**
     * Get a list of names for each input source.
     * This is intended to be reimplemented by widgets supporting
     * more than one input source
     */
    virtual QStringList inputSourceNames() const;
>>>>>>>

    /**
     * Add an external input source. This is used when a widget supports
     * more than one input source.
     *
     * @param source The input source to add
     */
    void addInputSource(QSharedPointer<QLCInputSource> source);

    /**
     * Update an already existing input source with new parameters
     *
     * @param source The input source to update
     * @param universe The new universe
     * @param channel The new channel
     */
    void updateInputSource(QSharedPointer<QLCInputSource> source, quint32 universe, quint32 channel);
