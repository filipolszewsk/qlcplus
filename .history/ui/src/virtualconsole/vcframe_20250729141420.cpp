...
QStringList VCFrame::inputSourceNames() const
{
    QStringList list;
    list << tr("Enable");
    if (m_multiPageMode)
    {
        list << tr("Next Page");
        list << tr("Previous Page");

        for (int i = 0; i < m_pageShortcuts.count(); i++)
        {
            if (m_pageShortcuts.at(i)->name().isEmpty())
                list << tr("Page %1").arg(i + 1);
            else
                list << m_pageShortcuts.at(i)->name();
        }
    }
