    ...
    , m_widgets(widgets)
{
    ...
    connect(m_ui->m_buttonBox, &QDialogButtonBox::accepted, this, &VCMultiInputEditor::accept);
    connect(m_ui->m_buttonBox, &QDialogButtonBox::rejected, this, &VCMultiInputEditor::reject);
...
void VCMultiInputEditor::updateItem(QTreeWidgetItem* item)
{
    ...
    m_ui->treeWidget->setItemWidget(item, 1, universeSpin);

    QSpinBox* channelSpin = new QSpinBox(this);
    channelSpin->setRange(0, 511);
    m_ui->treeWidget->setItemWidget(item, 2, channelSpin);
}
...
void VCMultiInputEditor::accept()
{
    for (int i = 0; i < m_ui->treeWidget->topLevelItemCount(); ++i)
    {
        ...
        QSpinBox* universeSpin = qobject_cast<QSpinBox*>(m_ui->treeWidget->itemWidget(item, 1));
        QSpinBox* channelSpin = qobject_cast<QSpinBox*>(m_ui->treeWidget->itemWidget(item, 2));
