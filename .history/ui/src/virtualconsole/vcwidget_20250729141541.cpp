...
QString VCWidget::typeToString(int type)
{
    switch (type)
    {
        ...
        case MultiInputWidget: return tr("Multi Input");
        case UnknownWidget:
        default:
...
QStringList VCWidget::inputSourceNames(int type)
{
    switch (type)
    {
        case ButtonWidget: return VCButton::inputSourceNames();
        case SliderWidget: return VCSlider::inputSourceNames();
        case XYPadWidget: return VCXYPad::inputSourceNames();
        case FrameWidget: return VCFrame::inputSourceNames();
        case SoloFrameWidget: return VCFrame::inputSourceNames();
        case SpeedDialWidget: return VCSpeedDial::inputSourceNames();
        case CueListWidget: return VCCueList::inputSourceNames();
        case AudioTriggersWidget: return VCAudioTriggers::inputSourceNames();
        case AnimationWidget: return VCMatrix::inputSourceNames();
        case ClockWidget: return VCClock::inputSourceNames();
        case MultiInputWidget:
        {
            VCMultiInput* multiInput = qobject_cast<VCMultiInput*>(this);
            if (multiInput)
                return multiInput->inputSourceNames();
            return QStringList();
        }
        default:
             return QStringList();
...
QIcon VCWidget::typeToIcon(int type)
{
    switch (type)
    {
        ...
        case MultiInputWidget: return QIcon(":/multitrack.png");
        case UnknownWidget:
        default:
...
{
    return m_inputs.values();
}

int VCWidget::inputSourcesCount() const
{
    return m_inputs.count();
}
>>>>>>>

QStringList VCWidget::inputSourceNames() const
{
    return QStringList();
}

void VCWidget::addInputSource(QSharedPointer<QLCInputSource> source)
{
    if (source.isNull() || source->isValid() == false)
        return;

    quint8 newID = 0;
    if (m_inputs.isEmpty() == false)
        newID = m_inputs.keys().last() + 1;

    setInputSource(source, newID);
}

void VCWidget::updateInputSource(QSharedPointer<QLCInputSource> source, quint32 universe, quint32 channel)
{
    if (source.isNull() || source->isValid() == false)
        return;
    ...
    for (quint8 id : m_inputs.keys())
    {
        if (m_inputs.value(id) == source)
        {
            setInputSource(source, id);
            return;
        }
    }
}
...
void VCWidget::invokeMenu(const QPoint& point)
{
    ...
    // Add a custom properties entry here,
    // since we cannot do it in VirtualConsole
    if (vc->selectedWidgets().size() > 1)
    {
        QAction *action = new QAction(QIcon(":/multitrack.png"), tr("Multi-Input Properties"), menu);
        connect(action, SIGNAL(triggered(bool)),
                vc, SLOT(slotEditMultiInputProperties()));
        menu->insertAction(vc->customMenu()->menuAction(), action);
        menu->insertSeparator(vc->customMenu()->menuAction());
    }
