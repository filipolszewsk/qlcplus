...
void VCMultiPatchEditor::accept()
{
    ...
    while (it.hasNext())
    {
        ...
        quint32 address = item->data(1, Qt::EditRole).toUInt();
        ...
        if (widget->type() == VCWidget::SliderWidget)
        {
            ...
            if (slider)
            {
                ...
                if (widget->inputSource(VCSlider::sliderInputSourceId))
                    sliderItem->setData(1, Qt::EditRole, widget->inputSource(VCSlider::sliderInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCSlider::overrideResetInputSourceId))
                    resetItem->setData(1, Qt::EditRole, widget->inputSource(VCSlider::overrideResetInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCSlider::flashButtonInputSourceId))
                    flashItem->setData(1, Qt::EditRole, widget->inputSource(VCSlider::flashButtonInputSourceId)->toUInt());
            }
        }
        else if (widget->type() == VCWidget::ButtonWidget)
        {
            ...
            if (button)
            {
                ...
                if (widget->inputSource(0))
                    buttonItem->setData(1, Qt::EditRole, widget->inputSource(0)->toUInt());
            }
        }
        else if (widget->type() == VCWidget::CueListWidget)
        {
            ...
            if (cueList)
            {
                ...
                if (widget->inputSource(VCCueList::nextInputSourceId))
                    nextItem->setData(1, Qt::EditRole, widget->inputSource(VCCueList::nextInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCCueList::previousInputSourceId))
                    prevItem->setData(1, Qt::EditRole, widget->inputSource(VCCueList::previousInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCCueList::playbackInputSourceId))
                    playbackItem->setData(1, Qt::EditRole, widget->inputSource(VCCueList::playbackInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCCueList::stopInputSourceId))
                    stopItem->setData(1, Qt::EditRole, widget->inputSource(VCCueList::stopInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCCueList::sideFaderInputSourceId))
                    faderItem->setData(1, Qt::EditRole, widget->inputSource(VCCueList::sideFaderInputSourceId)->toUInt());
            }
        }
        else if (widget->type() == VCWidget::FrameWidget)
        {
            ...
            if (frame)
            {
                ...
                if (widget->inputSource(VCFrame::enableInputSourceId))
                    enableItem->setData(1, Qt::EditRole, widget->inputSource(VCFrame::enableInputSourceId)->toUInt());
                ...
                if (frame->multipageMode())
                {
                    ...
                    if (widget->inputSource(VCFrame::nextPageInputSourceId))
                        nextItem->setData(1, Qt::EditRole, widget->inputSource(VCFrame::nextPageInputSourceId)->toUInt());
                    ...
                    if (widget->inputSource(VCFrame::previousPageInputSourceId))
                        prevItem->setData(1, Qt::EditRole, widget->inputSource(VCFrame::previousPageInputSourceId)->toUInt());
                }
            }
        }
        else if (widget->type() == VCWidget::XYPadWidget)
        {
            ...
            if (pad)
            {
                ...
                if (widget->inputSource(VCXYPad::panInputSourceId))
                    panItem->setData(1, Qt::EditRole, widget->inputSource(VCXYPad::panInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCXYPad::tiltInputSourceId))
                    tiltItem->setData(1, Qt::EditRole, widget->inputSource(VCXYPad::tiltInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCXYPad::widthInputSourceId))
                    widthItem->setData(1, Qt::EditRole, widget->inputSource(VCXYPad::widthInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCXYPad::heightInputSourceId))
                    heightItem->setData(1, Qt::EditRole, widget->inputSource(VCXYPad::heightInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCXYPad::panFineInputSourceId))
                    panFineItem->setData(1, Qt::EditRole, widget->inputSource(VCXYPad::panFineInputSourceId)->toUInt());
                ...
                if (widget->inputSource(VCXYPad::tiltFineInputSourceId))
                    tiltFineItem->setData(1, Qt::EditRole, widget->inputSource(VCXYPad::tiltFineInputSourceId)->toUInt());
            }
        }
