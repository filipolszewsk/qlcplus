...
const quint8 VCCueList::stopInputSourceId = 3;
const quint8 VCCueList::crossfaderInputSourceId = 4;
const quint8 VCCueList::sideFaderInputSourceId = 5;
...
void VCCueList::updateStepList()
{
    ...
    m_tree->header()->resizeSections(QHeaderView::Stretch);
...
void VCCueList::updateFeedback()
{
    ...
    // Next cue
    sendFeedback(m_nextLatestValue, nextInputSourceId);

    // Previous cue
    sendFeedback(m_previousLatestValue, previousInputSourceId);

    // Playback
    sendFeedback(m_playbackLatestValue, playbackInputSourceId);

    // Stop
    sendFeedback(m_stopLatestValue, stopInputSourceId);

    // Side fader
    if (isSideFaderVisible())
        sendFeedback(m_sideFader->value(), crossfaderInputSourceId);
}
...
void VCCueList::slotInputValueChanged(quint32 universe, quint32 channel, uchar value)
{
    ...
    else if (checkInputSource(universe, pagedCh, value, sender(), crossfaderInputSourceId))
    {
...
bool VCCueList::loadXML(QXmlStreamReader &root)
{
    ...
    while (root.readNextStartElement())
    {
        ...
        {
            loadXMLSources(root, crossfaderInputSourceId);
        }
        else if (root.name() == KXMLQLCVCCueListSideFader)
        {
...
bool VCCueList::saveXML(QXmlStreamWriter *doc)
{
    ...
    QSharedPointer<QLCInputSource> cf1Src = inputSource(crossfaderInputSourceId);
    ...
    QSharedPointer<QLCInputSource> sfSrc = inputSource(sideFaderInputSourceId);
    if (!sfSrc.isNull() && sfSrc->isValid())
    {
        doc->writeStartElement(KXMLQLCVCCueListSideFader);
        saveXMLInput(doc, sfSrc);
        doc->writeEndElement();
    }
>>>>>>>
