...
static const QString KXMLEntryInputValue   = QStringLiteral("InputValue");
...
static const QString KXMLLevelPresetFlashOverride = QStringLiteral("FlashOverride");
static const QString KXMLLevelPresetFlashForceLTP = QStringLiteral("FlashForceLTP");
...
static const QString KXMLFunctionFlashOverride = QStringLiteral("FlashOverride");
static const QString KXMLFunctionFlashForceLTP = QStringLiteral("FlashForceLTP");
...
static EntryInputBinding readInputBlock(QXmlStreamReader& root, VCWidget* widget)
{
    ...
    if (attrs.hasAttribute(KXMLEntryInputValue))
        binding.inputValue = qBound(0, attrs.value(KXMLEntryInputValue).toInt(), 255);
...
static void saveInputBlock(QXmlStreamWriter* doc, const QSharedPointer<QLCInputSource>& src,
                           const QKeySequence& key, int inputValue = -1)
{
    if (inputValue >= 0)
        doc->writeAttribute(KXMLEntryInputValue, QString::number(qBound(0, inputValue, 255)));
    if (!src.isNull() && src->isValid())
...
void MultiButtonWidget::setWidgetMode(MultiButtonMode mode)
{
    ...
    m_monitorDisplayIndex = -1;
    m_lastMonitorMatchIdx = -1;
    m_pendingMonitorMatchIdx = -2;
    m_pendingMonitorMatchCount = 0;
                                   ...
                                   const QStringList&    iconPaths)
{
    ...
    m_entryInputValueMatched.clear();
    ...
    while (m_functionEntryInputValues.size() < m_functionIds.size())
    ...
    while (m_functionEntryFlashOverride.size() < m_functionIds.size())
        m_functionEntryFlashOverride.append(false);
    while (m_functionEntryFlashOverride.size() > m_functionIds.size())
        m_functionEntryFlashOverride.removeLast();
    while (m_functionEntryFlashForceLtp.size() < m_functionIds.size())
...
int MultiButtonWidget::entryInputValueSource(int idx) const
{
    if (idx < 0)
        return -1;
    ...
    if (m_mode == MultiButtonMode::Level)
    {
        if (idx >= m_levelPresets.size())
            return -1;
        return m_levelPresets.at(idx).entryInputValue;
    }
    if (m_mode == MultiButtonMode::Widget)
    {
        if (idx >= m_widgetEntryAppearance.size())
            return -1;
        return m_widgetEntryAppearance.at(idx).entryInputValue;
    }

    if (idx >= m_functionEntryInputValues.size())
        return -1;
    return m_functionEntryInputValues.at(idx);
}

void MultiButtonWidget::setEntryInputValueSource(int idx, int value)
{
    if (idx < 0)
        return;
    ...
    if (m_mode == MultiButtonMode::Level)
    {
        if (idx >= m_levelPresets.size())
            return;
        m_levelPresets[idx].entryInputValue = normalized;
    }
    else if (m_mode == MultiButtonMode::Widget)
    {
        syncWidgetEntryAppearanceCount();
        if (idx < m_widgetEntryAppearance.size())
            m_widgetEntryAppearance[idx].entryInputValue = normalized;
    }
    else
    {
        while (m_functionEntryInputValues.size() <= idx)
            m_functionEntryInputValues.append(-1);
        m_functionEntryInputValues[idx] = normalized;
    }
}
...
void MultiButtonWidget::syncDynamicEntryCountLayout()
{
    ...
    if (m_monitorMatchIndex >= count)
        m_monitorMatchIndex = -1;
    if (m_monitorDisplayIndex >= count)
        m_monitorDisplayIndex = -1;
    if (m_lastMonitorMatchIdx >= count)
        m_lastMonitorMatchIdx = -1;
    if (m_pendingMonitorMatchIdx >= count)
    {
        m_pendingMonitorMatchIdx = -2;
        m_pendingMonitorMatchCount = 0;
        m_monitorNoMatchCount = 0;
...
QVector<SpreadTileInfo> MultiButtonWidget::computeSpreadTiles() const
{
    ...
    if (spreadPagingActive())
    {
        ...
        for (int local = 0; local < spp && (pageStart + local) < totalAll; ++local)
        {
            ...
            SpreadTileInfo info;
            const bool isOff = offSlotAvailable() && (globalSlot == totalAll - 1);
    ...
    for (int slot = 0; slot < qMin(totalAll, maxSlots); ++slot)
    {
        ...
        SpreadTileInfo info;
        const bool isOff = offSlotAvailable() && (slot == totalAll - 1);
...
bool MultiButtonWidget::widgetLinkStateSyncNeeded() const
{
    return m_mode == MultiButtonMode::Widget
            && m_widgetTargetId != VCWidget::invalidId()
...
void MultiButtonWidget::ensureChannelMonitorTimer()
{
    if (m_channelMonitorTimer)
        return;
...
void MultiButtonWidget::updateChannelMonitorTimerInterval()
{
    if (!m_channelMonitorTimer)
        return;
    const bool widgetStateSync = widgetLinkStateSyncNeeded();
    const bool fastSingleLevelDisplay = m_layout == MultiButtonLayout::Single
            && m_mode == MultiButtonMode::Level
            && m_monitorChannelValues
            && !stagingActive()
            && !widgetLinkUsesInternalStaging();
    const int intervalMs = widgetStateSync ? 40
            : (fastSingleLevelDisplay ? 40 : (stagingActive() ? 75 : 100));
    m_channelMonitorTimer->setInterval(intervalMs);
}

void MultiButtonWidget::updateChannelMonitorTimerState()
{
    const bool needed = m_monitorChannelValues || widgetLinkStateSyncNeeded();
...
{
    return m_addOffAtEnd || widgetLinkHasImplicitOff();
}
...
void MultiButtonWidget::writeDMX(MasterTimer* /*timer*/, QList<Universe*> universes)
{
    ...
    const bool flashHoldActive = m_flashHoldIndex == dmxIdx && preset.flashOnActivate;
    const Universe::FaderPriority priority = (flashHoldActive && preset.flashOverride)
            ? Universe::Override : Universe::Auto;
    const bool forceLtp = flashHoldActive && preset.flashForceLtp;
    ...
    for (int i = 0; i < m_levelChannelBindings.size() && i < resolved.size(); ++i)
    {
        ...
        if (fader.isNull())
        {
            fader = universes.at(universe)->requestFader(priority);
            fader->adjustIntensity(intensity());
            m_fadersMap[universe] = fader;
        }
        else if (fader->priority() != priority)
        {
            universes.at(universe)->requestFaderPriority(fader, priority);
        }
...
void MultiButtonWidget::beginFlashHold(int idx)
{
    ...
    if (m_mode == MultiButtonMode::Function)
    {
        ...
        {
            const bool flashOverride = idx < m_functionEntryFlashOverride.size()
                    && m_functionEntryFlashOverride.at(idx);
            const bool flashForceLtp = idx < m_functionEntryFlashForceLtp.size()
                    && m_functionEntryFlashForceLtp.at(idx);
            f->flash(m_doc->masterTimer(), flashOverride, flashForceLtp);
        }
    }
    else
    {
...
static void drawFlashHoldBorder(QPainter& p, const QRect& rect)
{
    p.save();
    p.setRenderHint(QPainter::Antialiasing, true);
    p.setBrush(Qt::NoBrush);
    p.setPen(QPen(QColor(255, 45, 45, 245), 3));
    p.drawRoundedRect(rect.adjusted(3, 3, -4, -4), 5, 5);
    p.restore();
}
...
private:
    ...
    int rowCount() const
    {
        ...
        if (n <= 0)
            return 0;
        return n + (m_owner->offSlotAvailable() ? 1 : 0);
    }
...
void MultiButtonWidget::cycleNext()
{
    ...
    {
        activate(next, m_layout == MultiButtonLayout::Single);
    }
}

void MultiButtonWidget::setAutomationEnabled(bool enable)
{
    if (m_automationEnabled == enable)
    {
        if (!enable && m_automationProfileSelectorValue != 0)
        {
            m_automationProfileSelectorValue = 0;
            bumpAutomationProfileTargetRevision();
            update();
        }
        return;
    }
...
QString MultiButtonWidget::targetDisplayName() const
{
    const QString listName = m_targetListName.trimmed();
    if (!listName.isEmpty())
        return listName;
...
bool MultiButtonWidget::automationProfileTargetValid(int outputIdx, int parameter) const
{
    return outputIdx == 0 && parameter == 0;
}

void MultiButtonWidget::bumpAutomationProfileTargetRevision()
{
    ++m_automationProfileTargetRevision;
    if (m_automationProfileTargetRevision == 0)
        m_automationProfileTargetRevision = 1;
}

int MultiButtonWidget::multiButtonOutputCount() const
{
    return 1;
}

QString MultiButtonWidget::multiButtonOutputName(int outputIdx) const
{
    return outputIdx == 0 ? tr("Automation") : QString();
}

int MultiButtonWidget::multiButtonParameterCount() const
{
    return 1;
}

QString MultiButtonWidget::multiButtonParameterName(int parameter) const
{
    return parameter == 0 ? tr("Automation profile") : QString();
}

int MultiButtonWidget::multiButtonEntryCount(int outputIdx, int parameter) const
{
    return automationProfileTargetValid(outputIdx, parameter)
            ? m_automationProfiles.size() : 0;
}

QString MultiButtonWidget::multiButtonEntryName(int outputIdx, int parameter, int index) const
{
    if (!automationProfileTargetValid(outputIdx, parameter)
            || index < 0 || index >= m_automationProfiles.size())
        return QString();
    const QString name = m_automationProfiles.at(index).name.trimmed();
    return name.isEmpty() ? tr("Profile %1").arg(index + 1) : name;
}

int MultiButtonWidget::multiButtonCurrentIndex(int outputIdx, int parameter) const
{
    return multiButtonLiveIndex(outputIdx, parameter);
}

int MultiButtonWidget::multiButtonLiveIndex(int outputIdx, int parameter) const
{
    if (!automationProfileTargetValid(outputIdx, parameter)
            || m_automationSuspended
            || m_automationProfileSelectorValue == 0
            || m_automationProfiles.isEmpty())
        return -1;
    return qBound(0, m_activeAutomationProfile, m_automationProfiles.size() - 1);
}

bool MultiButtonWidget::multiButtonStagingAvailable(int outputIdx, int parameter) const
{
    Q_UNUSED(outputIdx);
    Q_UNUSED(parameter);
    return false;
...
quint64 MultiButtonWidget::multiButtonStateRevision(int outputIdx, int parameter) const
{
    return automationProfileTargetValid(outputIdx, parameter)
            ? m_automationProfileTargetRevision : 0;
}

bool MultiButtonWidget::multiButtonHasStagedIndex(int outputIdx, int parameter) const
{
    Q_UNUSED(outputIdx);
    Q_UNUSED(parameter);
    return false;
...
int MultiButtonWidget::multiButtonStagedIndex(int outputIdx, int parameter) const
{
    Q_UNUSED(outputIdx);
    Q_UNUSED(parameter);
    return -1;
...
bool MultiButtonWidget::multiButtonActivate(int outputIdx, int parameter, int index)
{
    if (!automationProfileTargetValid(outputIdx, parameter))
        return false;
    if (index < -1 || index >= m_automationProfiles.size())
        return false;

    handlePresetChooseInput(selectorIndexToBusValue(index));
    if (m_doc)
        m_doc->setModified();
    return true;
}

bool MultiButtonWidget::multiButtonActivateStaged(int outputIdx, int parameter, int index)
{
    Q_UNUSED(outputIdx);
    Q_UNUSED(parameter);
    Q_UNUSED(index);
...
QSharedPointer<QLCInputSource> MultiButtonWidget::multiButtonLiveInputSource(int outputIdx,
                                                                             int parameter) const
{
    if (!automationProfileTargetValid(outputIdx, parameter))
        return QSharedPointer<QLCInputSource>();
    return inputSource(presetChooseInputSourceId);
}

bool MultiButtonWidget::multiButtonSetLiveInputSource(int outputIdx, int parameter,
                                                      QSharedPointer<QLCInputSource> src)
{
    if (!automationProfileTargetValid(outputIdx, parameter))
        return false;
    setInputSource(src, presetChooseInputSourceId);
    if (m_doc)
        m_doc->setModified();
    return true;
}
...
void MultiButtonWidget::slotCheckChannelValues()
{
    ...
    const bool fastSingleLevelDisplay = m_layout == MultiButtonLayout::Single
            ...
            && !stagingActive()
            && !widgetLinkUsesInternalStaging();
    const int debounceMs = fastSingleLevelDisplay ? 0 : (stagingActive() ? 100 : 100);
    ...
    if (m_mode == MultiButtonMode::Function)
    {
        ...
        if (!matches.isEmpty())
        {
            if (fastSingleLevelDisplay
                    && m_monitorDisplayIndex >= 0
                    && matches.contains(m_monitorDisplayIndex))
                matchIdx = m_monitorDisplayIndex;
            else if (fastSingleLevelDisplay
                     && m_monitorMatchIndex >= 0
                     && matches.contains(m_monitorMatchIndex))
                matchIdx = m_monitorMatchIndex;
            else if (!fastSingleLevelDisplay
                     && m_currentIndex >= 0
                     && matches.contains(m_currentIndex))
    ...
    if (matchIdx >= 0)
        ...
        ++m_monitorNoMatchCount;

    if (matchIdx == m_pendingMonitorMatchIdx)
        ++m_pendingMonitorMatchCount;
    else
    {
        m_pendingMonitorMatchIdx = matchIdx;
        m_pendingMonitorMatchCount = 1;
    }
    ...
    const int prevMonitorDisplay = m_monitorDisplayIndex;

    if (matchIdx >= 0 && stagingActive() && hasLocalStagedSelection()
            && matchIdx != m_lastMonitorMatchIdx)
    {
    ...
    else if (matchIdx < 0 && m_monitorNoMatchCount >= noMatchGraceCount)
    {
        m_monitorDisplayIndex = -1;
    }

    if (m_monitorMatchIndex != prevMonitorMatch
            || m_monitorDisplayIndex != prevMonitorDisplay
            || stagedCleared)
        update();
}
...
int MultiButtonWidget::displayedEntryIndex() const
{
    ...
    if (m_layout == MultiButtonLayout::Single
            && m_monitorChannelValues
            && m_mode != MultiButtonMode::Widget
            && !stagingActive()
            && m_monitorDisplayIndex >= 0)
    {
        return m_monitorDisplayIndex;
    }
    return m_currentIndex;
}
...
bool MultiButtonWidget::syncWidgetLinkLiveStagedState()
{
    ...
    const bool changed = m_currentIndex != prevLive
            ...
            || m_widgetTargetStateRevision != prevRevision;
...
void MultiButtonWidget::commitEntrySelectPreview()
{
    ...
    activate(idx, m_layout == MultiButtonLayout::Single);
}
...
{
    const uchar previousValue = m_automationProfileSelectorValue;
    const bool previousSuspended = m_automationSuspended;
    const int previousProfile = m_activeAutomationProfile;

    m_automationProfileSelectorValue = value;
    if (value == 0)
    {
        m_automationSuspended = true;
        if (previousValue != m_automationProfileSelectorValue
                || previousSuspended != m_automationSuspended
                || previousProfile != m_activeAutomationProfile)
            bumpAutomationProfileTargetRevision();
...
void MultiButtonWidget::slotInputValueChanged(quint32 universe, quint32 channel, uchar value)
{
    ...
    const int entries = qMin(entryCount(), MBInputId::kMaxEntryInputs);
    while (m_entryInputValueMatched.size() < entries)
        m_entryInputValueMatched.append(false);
    while (m_entryInputValueMatched.size() > entries)
        m_entryInputValueMatched.removeLast();

    bool matchedEntryInputSource = false;
    for (int i = 0; i < entries; ++i)
    {
        if (checkInputSource(universe, pagedCh, value, sender(), MBInputId::entryTrigger(i)))
        {
            matchedEntryInputSource = true;
            const int requiredValue = entryInputValueSource(i);
            const bool exactValue = requiredValue >= 0;
            const bool matchedValue = exactValue
                    ? int(value) == requiredValue
                    : value > 0;
            const bool wasMatched = i < m_entryInputValueMatched.size()
                    && m_entryInputValueMatched.at(i);
            ...
            if (entryIsFlash(i))
            {
                if (matchedValue)
                    beginFlashHold(i);
                else if (!exactValue || (wasMatched && m_flashHoldIndex == i))
                    endFlashHold();
            }
...
void MultiButtonWidget::slotKeyReleased(const QKeySequence& keySequence)
{
    if (!acceptsInput())
        return;
...
void MultiButtonWidget::editProperties()
{
    ...
    MultiButtonConfigDialog dlg(
        ...
        m_functionEntryKeys,
        m_functionEntryInputValues,
        m_spreadSlotInputs,
        m_spreadSlotKeys,
        m_functionEntryFlash,
        m_functionEntryFlashOverride,
        m_functionEntryFlashForceLtp,
    ...
    m_targetListName = dlg.targetListName();
    ...
    m_functionEntryFlashOverride = dlg.functionEntryFlashOverride();
    m_functionEntryFlashForceLtp = dlg.functionEntryFlashForceLtp();
    ...
    m_functionEntryInputValues = dlg.functionEntryInputValues();
    while (m_functionEntryInputValues.size() < m_functionIds.size())
        m_functionEntryInputValues.append(-1);
    while (m_functionEntryInputValues.size() > m_functionIds.size())
        m_functionEntryInputValues.removeLast();
...
VCWidget* MultiButtonWidget::createCopy(VCWidget* parent)
{
    ...
    copy->m_functionEntryInputValues = m_functionEntryInputValues;
    for (int i = 0; i < m_functionEntryInputs.size(); ++i)
        copy->setEntryInputSource(i, cloneInputSource(m_functionEntryInputs.at(i)));
    for (int i = 0; i < m_functionEntryKeys.size(); ++i)
        copy->setEntryKeySource(i, m_functionEntryKeys.at(i));
    for (int i = 0; i < m_functionEntryInputValues.size(); ++i)
        copy->setEntryInputValueSource(i, m_functionEntryInputValues.at(i));
    ...
    for (LevelPreset& preset : copy->m_widgetEntryAppearance)
        preset.entryInput = cloneInputSource(preset.entryInput);
    copy->syncWidgetLinkLiveStagedState();
    copy->updateChannelMonitorTimerState();
    ...
    copy->m_functionEntryFlashOverride = m_functionEntryFlashOverride;
    copy->m_functionEntryFlashForceLtp = m_functionEntryFlashForceLtp;
    ...
    if (m_mode == MultiButtonMode::Function)
    {
        ...
        for (int i = 0; i < n; ++i)
        {
            if (src->m_mode == MultiButtonMode::Function)
            {
                ...
                if (i < src->m_functionEntryFlash.size()
                    && i < m_functionEntryFlash.size())
                    m_functionEntryFlash[i] = src->m_functionEntryFlash.at(i);
                if (i < src->m_functionEntryFlashOverride.size()
                    && i < m_functionEntryFlashOverride.size())
                    m_functionEntryFlashOverride[i] = src->m_functionEntryFlashOverride.at(i);
                if (i < src->m_functionEntryFlashForceLtp.size()
                    && i < m_functionEntryFlashForceLtp.size())
                    m_functionEntryFlashForceLtp[i] = src->m_functionEntryFlashForceLtp.at(i);
            }
    ...
    else if (m_mode == MultiButtonMode::Widget)
    {
        ...
        for (int i = 0; i < n && i < m_widgetEntryAppearance.size(); ++i)
        {
            ...
            if (s)
            {
                ...
                tgt.flashOnActivate = s->flashOnActivate;
                tgt.flashOverride = s->flashOverride;
                tgt.flashForceLtp = s->flashForceLtp;
            }
            else
            {
...
void MultiButtonWidget::applyFunctionAssignmentsFrom(const MultiButtonWidget* src)
{
    ...
    m_functionEntryFlash = src->m_functionEntryFlash;
    m_functionEntryFlashOverride = src->m_functionEntryFlashOverride;
    m_functionEntryFlashForceLtp = src->m_functionEntryFlashForceLtp;
    while (m_functionEntryFlash.size() < m_functionIds.size())
        m_functionEntryFlash.append(false);
    while (m_functionEntryFlash.size() > m_functionIds.size())
        m_functionEntryFlash.removeLast();
...
void MultiButtonWidget::toClipboardJson(QJsonObject &obj, const Doc *doc) const
{
    ...
    if (!m_targetListName.trimmed().isEmpty())
        obj["targetListName"] = m_targetListName.trimmed();
    ...
    for (int i = 0; i < m_functionIds.size(); ++i)
    {
        ...
        if (i < m_functionEntryFlash.size() && m_functionEntryFlash.at(i))
            entry["flash"] = true;
        if (i < m_functionEntryFlashOverride.size()
            && m_functionEntryFlashOverride.at(i))
            entry["flashOverride"] = true;
        if (i < m_functionEntryFlashForceLtp.size()
            && m_functionEntryFlashForceLtp.at(i))
    ...
    for (const LevelPreset& preset : m_levelPresets)
    {
        ...
        if (preset.flashOnActivate)
            po["flash"] = true;
        if (preset.flashOverride)
            po["flashOverride"] = true;
        if (preset.flashForceLtp)
            po["flashForceLTP"] = true;
        ...
        if (preset.entryInputValue >= 0)
            po["inputValue"] = preset.entryInputValue;
        presetArr.append(po);
    }
    ...
    for (int i = 0; i < m_functionEntryInputs.size(); ++i)
    {
        ...
        const QKeySequence key = (i < m_functionEntryKeys.size())
                                     ...
                                     : QKeySequence();
        const int inputValue = (i < m_functionEntryInputValues.size())
                ? m_functionEntryInputValues.at(i) : -1;
        if ((src.isNull() || !src->isValid()) && key.isEmpty()
            && inputValue < 0)
            continue;
        QJsonObject io;
...
void MultiButtonWidget::fromClipboardJson(const QJsonObject &obj, Doc *doc)
{
    ...
    m_targetListName = obj["targetListName"].toString().trimmed();
    ...
    QList<bool> loadedFlashOverride;
    QList<bool> loadedFlashForceLtp;
    QList<QColor> loadedLabelColors;
    for (const QJsonValue& v : obj["entries"].toArray())
    {
        QJsonObject e = v.toObject();
        loadedFlash.append(e["flash"].toBool(false));
        loadedFlashOverride.append(e["flashOverride"].toBool(false));
        loadedFlashForceLtp.append(e["flashForceLTP"].toBool(false));
    ...
    m_functionEntryFlashOverride = loadedFlashOverride;
    m_functionEntryFlashForceLtp = loadedFlashForceLtp;
    ...
    for (const QJsonValue& pv : obj["levelPresets"].toArray())
    {
        ...
        preset.flashOverride = po["flashOverride"].toBool(false);
        preset.flashForceLtp = po["flashForceLTP"].toBool(false);
        ...
        preset.entryInputValue = po["inputValue"].toInt(-1);
    ...
    if (obj.contains("functionEntryInputs"))
    {
        ...
        {
            QJsonObject io = arr.at(arrayIndex).toObject();
            const int i = io["index"].toInt(arrayIndex);
            if (i < 0)
                continue;
            ...
            while (m_functionEntryKeys.size() <= i)
                m_functionEntryKeys.append(QKeySequence());
            while (m_functionEntryInputValues.size() <= i)
                m_functionEntryInputValues.append(-1);
            ...
            m_functionEntryInputValues[i] = io["inputValue"].toInt(-1);
        }
    }
...
bool MultiButtonWidget::loadXML(QXmlStreamReader& root)
{
    ...
    while (root.readNextStartElement())
    {
        ...
        else if (root.name() == KXMLTargetListName)
        {
            m_targetListName = root.readElementText().trimmed();
        }
        ...
        else if (root.name() == KXMLLevelPreset)
        {
            ...
            preset.flashOverride = attrs.value(KXMLLevelPresetFlashOverride).toInt() != 0;
            preset.flashForceLtp = attrs.value(KXMLLevelPresetFlashForceLTP).toInt() != 0;
            ...
            while (root.readNextStartElement())
            {
                if (root.name() == KXMLEntryTriggerInput)
                {
                    ...
                    preset.entryInputValue = binding.inputValue;
                }
        ...
        else if (root.name() == KXMLFunction)
        {
            ...
            functionFlashOverrideLoaded.append(attrs.value(KXMLFunctionFlashOverride).toInt() != 0);
            functionFlashForceLtpLoaded.append(attrs.value(KXMLFunctionFlashForceLTP).toInt() != 0);
    ...
    m_functionEntryFlashOverride = functionFlashOverrideLoaded;
    m_functionEntryFlashForceLtp = functionFlashForceLtpLoaded;
    ...
    m_functionEntryInputValues.clear();
    for (int i = 0; i < m_functionIds.size(); ++i)
    {
        ...
        m_functionEntryInputValues.append(binding.inputValue);
    }
...
bool MultiButtonWidget::saveXML(QXmlStreamWriter* doc)
{
    ...
    if (!m_targetListName.trimmed().isEmpty())
        doc->writeTextElement(KXMLTargetListName, m_targetListName.trimmed());
    ...
    if (m_mode == MultiButtonMode::Level)
    {
        ...
        for (const LevelPreset& preset : m_levelPresets)
        {
            ...
            if (preset.flashOverride)
                doc->writeAttribute(KXMLLevelPresetFlashOverride, QStringLiteral("1"));
            if (preset.flashForceLtp)
                doc->writeAttribute(KXMLLevelPresetFlashForceLTP, QStringLiteral("1"));
            if (preset.labelColor.isValid())
    ...
    else if (m_mode == MultiButtonMode::Function)
    {
        for (int i = 0; i < m_functionIds.size(); ++i)
        {
            ...
            if (i < m_functionEntryFlashOverride.size()
                && m_functionEntryFlashOverride.at(i))
                doc->writeAttribute(KXMLFunctionFlashOverride, QStringLiteral("1"));
            if (i < m_functionEntryFlashForceLtp.size()
                && m_functionEntryFlashForceLtp.at(i))
                doc->writeAttribute(KXMLFunctionFlashForceLTP, QStringLiteral("1"));
            if (i < m_functionEntryLabelColors.size()
                && m_functionEntryLabelColors.at(i).isValid())
            {
    ...
    for (int i = 0; i < m_functionEntryInputs.size(); ++i)
    {
        ...
        saveInputBlock(doc, src, key, inputValue);
        doc->writeEndElement();
    }
...
bool MultiButtonWidget::automationVisualActive() const
{
    return m_automationEnabled
            && !m_automationSuspended
            && m_automationProfileSelectorValue > 0
            && activeAutomationProfilePtr() != nullptr;
}
...
void MultiButtonWidget::paintSpread(QPainter& p)
{
    ...
    const bool automationActive = automationVisualActive();
    const QColor automationRed(220, 35, 35, 150);
    const QColor automationOrange(255, 150, 0, 235);
    ...
    for (const SpreadTileInfo& tile : computeSpreadTiles())
    {
        ...
        if (automationActive)
        {
            p.save();
            p.setBrush(Qt::NoBrush);
            p.setPen(isLiveTile ? QPen(automationOrange, 2) : QPen(automationRed, 1));
...
void MultiButtonWidget::paintSingle(QPainter& p)
{
    ...
    const int displayIdx = displayedEntryIndex();
    const int monitorIdx = monitorHighlightIndex();
    const bool hasLiveDot = m_mode == MultiButtonMode::Widget || m_monitorChannelValues;
    const int liveDotIdx = hasLiveDot ? m_monitorMatchIndex : -1;
    const int stagedDotIdx = stagedHighlightIndex();
    const bool hasStagedDot = stagedHighlightValid();
    ...
    int n         = entryCount();
    int totalDots = n + (offSlotAvailable() ? 1 : 0);
    if (totalDots > 0)
    {
        if (totalDots <= 12)
        {
            ...
            {
                const bool isOffDot = (offSlotAvailable() && i == n);
                const bool isDisplayDot = isOffDot ? (displayIdx < 0)
                                                   : (i == displayIdx);
                const bool isLiveDot = hasLiveDot
                        && (isOffDot ? (liveDotIdx < 0) : (i == liveDotIdx));
                const bool isStagedDot = hasStagedDot
                        && (isOffDot ? (stagedDotIdx < 0) : (i == stagedDotIdx));
                const QColor dotColor = isLiveDot ? QColor(255, 150, 0)
                                      : (isStagedDot ? QColor(45, 210, 95) : fg);

                if (isOffDot)
                {
                    p.setBrush((isDisplayDot || isLiveDot || isStagedDot)
                               ? dotColor : fg.darker(200));
                    p.setPen(isLiveDot ? QPen(dotColor.lighter(130), 1)
                             : (isStagedDot ? QPen(dotColor.lighter(130), 1)
                             : (isDisplayDot ? QPen(fg, 1) : Qt::NoPen)));
                    p.drawRect(x0 + i * (dotSize + gap), y0, dotSize, dotSize);
                }
                else
                {
                    p.setBrush((isDisplayDot || isLiveDot || isStagedDot)
                               ? dotColor : fg.darker(180));
                    p.setPen((isLiveDot || isStagedDot)
                             ? QPen(dotColor.lighter(130), 1) : Qt::NoPen);
    ...
    {
        const bool liveDisplay = (m_mode == MultiButtonMode::Widget)
                && ((displayIdx < 0 && monitorIdx < 0) || displayIdx == monitorIdx);
        p.setBrush(Qt::NoBrush);
        p.setPen(liveDisplay ? QPen(QColor(255, 150, 0, 235), 2)
                             : QPen(QColor(220, 35, 35, 150), 1));
